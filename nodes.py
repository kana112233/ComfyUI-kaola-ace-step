...
if ACESTEP_AVAILABLE:
    ...
    # --------------------------------------------------------------------------------
    # MonkeyPatch LoRA training for ComfyUI compatibility
    # Uses lora_training.py module for cleaner code separation
    # --------------------------------------------------------------------------------
    try:
        from .lora_training import apply_all_training_patches
        apply_all_training_patches()
    except ImportError as e:
        print(f"[MonkeyPatch] Could not import lora_training module: {e}")
        import traceback
        traceback.print_exc()
...
class ACE_STEP_LORA_TRAIN(ACE_STEP_BASE):
    ...
    def train_lora(self, tensor_dir, checkpoint_dir, config_path, output_dir, 
        ...
        # -------------------------------------------------------------------------
        from .lora_training import SafeLoRATrainer
