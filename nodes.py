...
if ACESTEP_AVAILABLE:
    ...
    # --------------------------------------------------------------------------------
    # MonkeyPatch training to disable bf16 and use fp32 (fixes BFloat16 errors)
    # --------------------------------------------------------------------------------
    try:
        import torch
        # Always patch for ComfyUI compatibility (bf16 doesn't work in worker threads)
        print("[MonkeyPatch] Applying fp32 training patch for ComfyUI compatibility...")
        ...
        def patched_train_from_preprocessed(self, tensor_dir, training_state=None, resume_from=None):
            ...
            # Use fp32 precision (NOT bf16)
            print(f"[MonkeyPatch] Using fp32 precision (bf16 disabled)")
        ...
        print("[MonkeyPatch] Training patched to use fp32 without Fabric")
    except Exception as e:
